...
bargraph.CI(x.factor = depth, response = shoot.no, group = disease.treatment, data = jan2018.focal, ylab = "shoots per focal vase", legend = T, ylim = c(0,10))

model <-aov(shoot.no ~ diversity.treatment*disease.treatment, data = filter(jan2018.focal, diversity.treatment != "mix"))
anova(model)

#total length per vase
...
bargraph.CI(x.factor = diversity.treatment, response = alive, group = disease.treatment, data = jan2018.focal, ylab = "alive", legend = T, ylim = c(0,4))

#shoot number per plant per vase
